...
import { isDecodedEOASigner, isDecodedFullSigner, decodeSignature, compareAddr } from '@0xsodium/config'
...
export async function isValidSignature(
  ...
  if (erc1271Check === undefined) {
    ...
    return isValidSodiumUndeployedWalletSignature(address, digest, sig, walletContext, provider, chainId)
  }
...
export async function isValidSodiumUndeployedWalletSignature(
  address: string,
  digest: Uint8Array,
  sig: string,
  walletContext?: SodiumContext,
