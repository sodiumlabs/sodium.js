...
import { UserOperationMiddlewareCtx } from "userop";
import {
  IUserOperation,
  IUserOperationBuilder,
  UserOperationMiddlewareFn,
} from "userop";
const { OpToJSON } = Utils;
...
export class UserOperationBuilder implements IUserOperationBuilder {
  ...
  private asyncMiddlewareStack: Array<UserOperationMiddlewareFn> = []; 
  ...
  useAsyncMiddleware(fn: UserOperationMiddlewareFn) {
    this.asyncMiddlewareStack = [...this.asyncMiddlewareStack, fn];
    return this;
}

  resetMiddleware() {
    this.middlewareStack = [];
    this.asyncMiddlewareStack = [];
    return this;
  }
  ...
  async buildOp(entryPoint: string, chainId: BigNumberish) {
    ...
    await Promise.all(this.asyncMiddlewareStack.map(fn => fn(ctx)))
